...
/********************* GLOBAL VARIABLES *********************/
	...
	function test(){
		if(file_exists("../banner/banner.png"))
            echo "png exist", EOF;
        else if(file_exists("../banner/banner.jpg"))
            echo "jpg exist", EOF;
        else if(file_exists("../banner/banner.jpge"))
            echo "jpge exist", EOF;
        else
            echo "none", EOF;
	...
	public function getBannerUrl(){
        if(file_exists('../banner/banner.png'))
            $banner = 'http://hcenter.nethost.co.il/banner/banner.png';
        else if(file_exists('../banner/banner.jpg'))
            $banner = 'http://hcenter.nethost.co.il/banner/banner.jpg';
        array_push($this->json_array,array("url"=>$banner));
        $this->output();
    }
	...
	public function getCoupanCount()
	{
		if (isset($_REQUEST['category'])) {
			...
			if ($category == 0)
				...
				$query_con = "WHERE c_category = $category";
		}else
			$query_con = "GROUP BY c_category";

		$query = "SELECT COUNT(id) as count FROM `add_coupon` $query_con ORDER BY c_category";
	...
	public function getCoupan(){
		if(isset($_REQUEST['coupan']) && isset($_REQUEST['device_id']) ){
			...
			if($coupan == "Yes"){
				...
				$order_con = "ORDER BY c_order";

				$query = "SELECT * FROM add_coupon $query_con $order_con LIMIT $start, $end";
				...
				if($count > 0){
					...
					foreach($fetch as $f){
						...
						if(!empty($f['c_image']) && isset($f['c_image'])){
							$imgname = basename($f['c_image']);
							$f['c_image'] = IMGDIRURL.$f['c_image'];
							$f['c_thumb_image'] = IMGDIRURL. $coupon_path . "/thumb_" .$imgname;
						}
	...
	public function getCoupanThumbs(){
		if(isset($_REQUEST['coupan']) && isset($_REQUEST['category']) ){
			$coupan = $_REQUEST['coupan'];
			$category = intval($_REQUEST['category']);
			...
			if($coupan == "Yes"){
				...
				$order_con = "ORDER BY c_order";
				$query = "SELECT id,c_category,c_image FROM add_coupon $query_con $order_con";
				...
				if($count > 0){
					...
					array_push($this->json_array,array("status"=>"failure","msg"=>"no coupons"));
            				$this->output();
				}
			}else{
				array_push($this->json_array,array("status"=>"failure2"));
